(() => {
  ...
  function emitPromptChanged() {
    try {
      window.dispatchEvent(new CustomEvent('promptChanged', {
        detail: {
          hasPrompt: !!(document.getElementById(customPromptId)?.value?.trim() || selectedText)
        }
      }));
    } catch (e) {
      // no-op
  ...
  document.addEventListener('DOMContentLoaded', async () => {
    ...
    const customPromptArea = document.getElementById(customPromptId);
    if (customPromptArea) {
      customPromptArea.addEventListener('input', emitPromptChanged);
    }
